...
contract SupplyChain {
    ...
    event ProductFound(string product);
    event StateAdded(string info);
    ...
    function addState(uint _productId, string memory info) public returns (bool) {
    ...
    function searchProduct(uint _productId) public returns (bool) {
        ...
        for (uint256 j=0; j<allProducts[_productId].totalStates; j++){
            output=concat(output, concat("<br>",allProducts[_productId].positions[j].description));
        }
        emit ProductFound(output);
        return true;
